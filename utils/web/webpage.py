...
def node_num(col,value = 1):
    node_num = col.number_input('Number of nodes',min_value=1,max_value=100,value=value,key='n')
    return node_num

def edge_input(session,col,node_num:int,predict_connection = None):
    ...
    if predict_connection:
        session.edge_list = []
        for i in predict_connection.keys():
            for j in list(predict_connection[i]['connect']):
                if (i,j) not in session.edge_list and (j,i) not in session.edge_list:
                    session.edge_list.append((i,j))
...
def node_setting(session,node_num:int,out_node:list[int]):
    ...
    for i in range(node_num):
        if i == out_node[0]:
            ...
            v_value = ccol3.number_input(label='m3',value=45, key = f'v{i}')
        elif index % 3 == 1:
...
def image_sidebar(folder_path):
    # st.sidebar.title("示例图片")
    img = None
    if st.sidebar.checkbox('使用示例图片'):
        image_dic = _load_images_from_folder(folder_path)
        with st.sidebar:
            img = image_select(
                label='选择一张图片',
                images=image_dic['image'],
            )
    return img


def _load_images_from_folder(folder_path):
    image_dic = {'image':[],'file_path':[]}
    for filename in os.listdir(folder_path):
        if filename.endswith(('png', 'jpg', 'jpeg')):
            path = os.path.join(folder_path, filename)
            image_dic['file_path'].append(path)
            image_dic['image'].append(Image.open(path))
    return image_dic