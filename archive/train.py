...
class model_setting():
    ...
    def __init__(self, model: torch.nn.Module, dataset: Dataset, epochs: int, batch_size: int, lr: float = 1e-3, decay: float = 1e-1, alpha: float = 0.2):
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(
            self.optimizer, step_size=4, gamma=0.1)
    ...
    def train(self, save_path: str = None, print_info: bool = False):
        ...
        for epoch in range(self.epochs):
            ...
            self.scheduler.step()
            print(
                f'Epoch [{epoch+1}/{self.epochs}], Loss: {val_loss}, Accuracy: {val_accuracy}, Train Accuracy: {train_accuracy}')
            if val_accuracy > best_accuracy:
                ...
                if save_path:
                    ...
                    torch.save(self.best_model.state_dict(),
                               save_path + f'model_{val_accuracy:.2f}.pth')
    ...
    def train_step(self, print_info: bool = False):
        ...
        for step, (image, mask) in enumerate(tqdm(train_loader)):
            ...
            if print_info:
                print(
                    f'Step [{step+1}/{total_steps}], Loss: {loss.item()}, Accuracy: {accuracy}')
